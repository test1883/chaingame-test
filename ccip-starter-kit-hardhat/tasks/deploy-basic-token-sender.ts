...
task(`deploy-basic-token-sender`, `Deploys the BasicTokenSender smart contract`)
  ...
  .setAction(
    async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
      ...
      const basicTokenSenderFactory =
        await hre.ethers.getContractFactory("BasicTokenSender");
      const basicTokenSender = await basicTokenSenderFactory.deploy(
        routerAddress,
        linkAddress,
      );
      ...
      spinner.stop();
      console.log(
        `✅ Basic Token Sender deployed at address ${basicTokenSender.address} on the ${hre.network.name} blockchain`,
      );
    },
  );
